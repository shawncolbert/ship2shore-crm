...
export async function fetchDefaultPipeline() {
  ...
  const { data: opps, error: oErr } = await supabase
    .from('opportunities')
    .select('id, org_id, title, service_code, port, vehicle, value, deposit_amount, escort_fee, scheduled_at, stage_id, contact_id, status, billing_number, booking_number, cleared, paid, deposit_paid, paid_on_site, pickup_address, dropoff_address, vehicle_make, vehicle_model, vehicle_year, vehicle_vin, vehicle_type, vehicle_modification, vehicle_extended, vehicle_body_class, vehicle_gvwr, suggested_price, confirmed_price, source_board, board_order_number, assigned_dispatcher_id, assigned_driver_card_id, contacts!opportunities_contact_id_fkey(full_name, company, email, phone), assigned_dispatcher:contacts!opportunities_assigned_dispatcher_id_fkey(id, full_name, company), invoices(id, status, invoice_number, total, amount_due, created_at, kind)')
    .eq('pipeline_id', pipeline.id)
