...
const REPLY_SYSTEM = (orgName) => `You are drafting an email reply on behalf of ${orgName}, a business using this CRM to manage customer bookings and communications.
...
const QUALIFY_SYSTEM = `You summarize a brand-new inbound customer email in one concise sentence for a dispatcher's CRM notes: what service they appear to want and any other useful detail. Output only that one sentence, no preamble.`
...
// message, across every org (not just Ship2Shore -- each conversation
// already carries its own org_id, so this just keys off that per-row instead
// of a single hardcoded org), draft (never send) a suggested reply for a
// human to review. Also suggests a one-line lead-qualification note on a
// contact's very first inbound message, if they don't already have one.
// Never touches opportunities or stages -- suggestion only, everywhere.
export const handler = async () => {
  try {
    const { data: convos, error: convErr } = await admin
      .from('conversations')
      .select('id, org_id, contact_id, contacts(email, notes)')
      .eq('channel', 'email')
    ...
    for (const convo of convos || []) {
      ...
      if (!orgNameCache.has(convo.org_id)) {
        const { data: org } = await admin.from('organizations').select('name').eq('id', convo.org_id).maybeSingle()
        orgNameCache.set(convo.org_id, org?.name || 'Dispatch')
      }
      const orgName = orgNameCache.get(convo.org_id)
      ...
      if (!fromAddrCache.has(convo.org_id)) {
        const { data: gmailRow } = await admin.from('gmail_oauth_tokens').select('email').eq('org_id', convo.org_id).maybeSingle()
        fromAddrCache.set(convo.org_id, gmailRow?.email || null)
      }
      ...
      await admin.from('messages').insert({
        ...
        channel: 'email',
        body: draftBody,
        from_addr: fromAddrCache.get(convo.org_id),
