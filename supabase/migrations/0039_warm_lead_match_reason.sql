-- Report which field actually matched instead of one generic string, so the
-- Prospecting UI can tell an exact identity match (phone/email -- almost
-- certainly the same person, don't duplicate) from a fuzzy name match
-- (could be a different but related person, worth adding + linking).
